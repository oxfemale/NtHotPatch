...
NtDirect::NtDirect(unsigned int service_id, const char* service_name){
	...
	*(unsigned int*)(cstub + SERVICE_ID_OFFSET) = id;
	
	// Copy the stub code
	DWORD dwOldProtect = 0;
	VirtualProtect((LPVOID)exec_page, PAGE_SIZE, PAGE_EXECUTE_READWRITE, &dwOldProtect);
	memcpy((void*)exec_page, cstub, sizeof(cstub));
	VirtualProtect((LPVOID)exec_page, PAGE_SIZE, dwOldProtect, &dwOldProtect);
	this->ptr = exec_page;
